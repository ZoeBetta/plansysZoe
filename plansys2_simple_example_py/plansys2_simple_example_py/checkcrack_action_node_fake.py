...
class CheckcrackAction(ActionExecutorClient):
    ...
    def do_work(self):
        if self.progress_ < 0.3:
            self.progress_ += 0.05
            self.send_feedback(self.progress_, 'Checkcrack running')
        else:
            ...
            if (found_person):
              ...
              time.sleep(1)
              self.finish(True, 1.0, 'Finish check crack')
            else:
              self.send_feedback(60.0, 'Good')
              time.sleep(1)
              self.finish(True, 1.0, 'Finish check crack')

        self.get_logger().info('checking crack ... {}'.format(self.progress_))


def main(args=None):
    rclpy.init(args=args)

    node = CheckcrackAction()
    node.set_parameters([Parameter(name='action_name', value='checkcrack')])
