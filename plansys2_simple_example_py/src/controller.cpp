...
public:
    ...
    void init()
    {
        ...
        file_.open("my_log.txt", std::ios::app);
        if (!file_.is_open()) {
        RCLCPP_ERROR(get_logger(), "Failed to open log file");
                                }
    }
    ...
    void init_knowledge()
    {
        ...
        goal = "and(searched spot d1) (searched spot e1) (searched spot c1) (searched spot d1) (searched spot e1) (searched spot f1) (searched spot g1) (searched spot h1) (searched spot i1)"
         "(environment_checked d1) (environment_checked e1) (environment_checked c1) (environment_checked d1) (environment_checked e1) (environment_checked f1) (environment_checked g1) (environment_checked h1) (environment_checked i1)";
    ...
    void finish_plan()
    {
        ...
        problem_expert_->setGoal(plansys2::Goal("(" + goaly + ")"));
    ...
    {
        auto replan_init_time = std::chrono::high_resolution_clock::now();
        std::cout << "START OF EMERGENCYYYYY" << std::endl;
        executor_client_->cancel_plan_execution();
        //std::cout << "START OF EMERGENCY1" << std::endl;
        std::string location;
        auto preds = problem_expert_->getPredicates();
        //std::cout << "START OF EMERGENCYYYYY2" << std::endl;

        problem_expert_->addPredicate(plansys2::Predicate("(emergency spot)"));
        problem_expert_->removePredicate(plansys2::Predicate("(not_emergency spot)"));
 ...
 */
        ...
        //goaly = "and (robot_at spot battery_point)"; //(finished spot)";
        //problem_expert_->setGoal(plansys2::Goal("(" + goal + ")"));
        //std::cout << "dopo goal" << std::endl;

        auto domain = domain_expert_->getDomain();
        auto problem = problem_expert_->getProblem();
        //std::cout << "AAAAAAAAAAAAAAA" << std::endl;
        auto plan = planner_client_->getPlan(domain, problem);
        //std::cout << "prima di problem" << std::endl;

        std::cout << problem << std::endl;
        //file_ << problem << std::endl;
        //std::cout << "dopo i problemi" << std::endl;
        const auto &plan2 = plan.value();
        for (const auto &item : plan2.items)
        {
            std::cout << "Action: " << item.action << std::endl;
    ...
    void step()
    {
        ...
        file_ << "Time taken for first planning: " << replan_duration.count() << " milliseconds" << std::endl;
            ...
            {
                std::cout << "Plan execution started successfully" << std::endl;
                state_ = INIT;
            }
        }
        break;
        ...
        case INIT:
        {
            ...
            for (const auto &action_feedback : feedback.action_execution_status)
            {
                ...
                {   //std::cout << "DSFVRTVSFGBRWBGDVS" << std::endl;
                    std::string sent = action_feedback.message_status;
                    std::istringstream iss(sent);
                    std::string word;
                    std::vector<std::string> words;
                ...
                if (action_feedback.action == "checkgas")
                {   
                    std::string sent = action_feedback.message_status;
                    std::istringstream iss(sent);
                    std::string word;
            ...
            if (action_feedback.action == "checkstairs")
                {   
                    std::string sent = action_feedback.message_status;
                    std::istringstream iss(sent);
                    std::string word;
                    ...
                    {//std::cout<< words[0] << std::endl;
                        if (words[0] == "EmergencyStair" && first == true)
                        {
                            ...
                            auto replan_end_time = std::chrono::high_resolution_clock::now();
                            std::chrono::duration<double, std::milli> replan_duration = replan_end_time - replan_init_time;
                            std::cout << "STAIRS Time taken for replanning: " << replan_duration.count() << " milliseconds" << std::endl;
                            file_ << "Time taken for replanning: " << replan_duration.count() << " milliseconds" << std::endl;
                            executor_client_->start_plan_execution(plan.value());
                        }
                        else if (words[0] == "Good" && f_bat == true)
                ...
                {   
                    ...
                    {//std::cout<< words[0] << std::endl;
                        if (words[0] == "EmergencyRoom" && first == true)
                        {
                            ...
                            from = words[1];
                            problem_expert_->addPredicate(plansys2::Predicate("(is_free spot)"));
                            problem_expert_->addPredicate(plansys2::Predicate("(door_closed " + from + ")"));
                            problem_expert_->removePredicate(plansys2::Predicate("(room_open " + from + ")"));
                            old_goal = goal;
                            auto a = check_predicate();
                            std::cout << "Replanning" << std::endl;
...
/*                              std::string to_remove = "(environment_checked " + from + ")";
            ...
            if (action_feedback.action == "opendoor")
                {   
                    std::string sent = action_feedback.message_status;
                    std::istringstream iss(sent);
                    std::string word;
                    ...
                    if (words.size() == 2)
                    {//std::cout<< words[0] << std::endl;
                        if (words[0] == "EmergencyDooropen" && first == true)
                        {
                            first = false;
                            std::cout << "open door broken: " << action_feedback.completion << std::endl;
                            executor_client_->cancel_plan_execution();
                            std::string from;
                            from = words[1];
                            problem_expert_->addPredicate(plansys2::Predicate("(is_free spot)"));
                            problem_expert_->addPredicate(plansys2::Predicate("(door_blocked " + from + ")"));
                            problem_expert_->removePredicate(plansys2::Predicate("(room_open " + from + ")"));
                            old_goal = goal;
                            auto a = check_predicate();
                            std::cout << "Replanning" << std::endl;
                            file_ << "Replan because door blocked disconnected" << std::endl;
                            if (a == false)
                            {
                                problem_expert_->addPredicate(plansys2::Predicate("(robot_at spot " + current_position + ")"));
                            }
                ...
                if (action_feedback.action == "searchoutside")
                {   
                    std::string sent = action_feedback.message_status;
                    std::istringstream iss(sent);
                    std::string word;
                    ...
                    { //std::cout<< words[0] << std::endl;
                        ...
                        {
                            first = false;
                            std::cout << "EmergencySearchOutside: " << action_feedback.completion << std::endl;
                            file_ << "Replan because someone is inside a blocked door" << std::endl;
                            emergency_plan();
                        }
                        else if (words[0] == "Good" && f_bat == true)
                        {
        ...
        case FINISHED:
        {
            ...
            file_ << "FINISHED. Time taken: " << duration_time.count() << " seconds" << std::endl;
                if (file_.is_open()) {
      file_.close();
    }
            exit_ = true;
